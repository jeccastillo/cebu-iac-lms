...
if (other != 0) api = 'finance/transactions_per_term_other';
$(document).ready(function() {
    ...
    $('#chooseDate').daterangepicker({
        ...
        daterange = "?start=" + start.format('YYYY-MM-D') + '&end=' + end.format(
            'YYYY-MM-D');
        dtable.fnDraw(false);
    });
    var dtable = $('#subjects-table').dataTable({
            ...
            $.get(api_url + api + daterange, {
                ...
                $.ajax({
                    ...
                    'success': function(payee_data) {
                        callback({
                            recordsTotal: json.meta
                                .to,
                            recordsFiltered: json
                                .meta.total,
                        ...
                        $("#print_form").click(function(e) {
                            ...
                            form.method = "post";
                            form.action =
                                "<?php echo base_url() ?>excel/daily_collection_report";
                            form.dataType = "json";
                            const hiddenField =
                                document
                                .createElement(
        ...
        "aoColumnDefs": [{
            ...
            "mRender": function(data, type, row, meta) {
                ...
                if (row.ns_payee_id) {
                    financeViewer =
                        '<li><a href="<?php echo base_url(); ?>finance/ns_transactions/' +
                        row.ns_payee_id + '">Finance Viewer</a></li>';
                } else {
                    financeViewer =
                        '<li><a href="<?php echo base_url(); ?>finance/manualPay/' +
                        row.slug + '">Finance Viewer</a></li>';
                }
                return '<?php echo $d_open; ?>' + financeViewer +
                    '<li><a href="<?php echo base_url(); ?>finance/remove_or_print/' +
                    row.or_number + '">Delete OR Print</a></li>'
            ...
            "aTargets": [2],
            "mRender": function(data, type, row, meta) {
                return '<a class="cashier-id" rel="' + row.cashier_id +
                    '" href="#">' + row.cashier_id + '</a>';
            }
        }, {
            "aTargets": [4],
            "mRender": function(data, type, row, meta) {
                if (row.or_number != null) return String(row.or_number)
                    .padStart(5, '0');
                else return " ";
            }
        }, {
            "aTargets": [5],
            "mRender": function(data, type, row, meta) {
                if (row.invoice_number != null) return String(row
                    .invoice_number).padStart(5, '0');
                else return " ";
            }
        }, {
            "aTargets": [7],
            ...
            "mRender": function(data, type, row, meta) {
                if (row.remarks) {
                    if (row.remarks == "Paynamics") return row
                        .request_id;
        ...
        "fnDrawCallback": function() {
            $(".cashier-id").click(function(e) {
                ...
                $.ajax({
                    'url': '<?php echo base_url(); ?>finance/cashier_details/' +
                        id,
                    'method': 'get',
                    'dataType': 'json',
                    'success': function(data) {
                        var cashier_details = data
                            .cashier_data;
                        Swal.fire({
                            title: "Cashier",
                            text: cashier_details
                                .strFirstname +
                                " " +
                                cashier_details
                                .strLastname,
            ...
            $("#select-filter").change(function(e) {
                ...
                document.location =
                    "<?php echo base_url().'finance/payments/0/0/' ?>" +
                    type_filter;
            });
        },
    });
