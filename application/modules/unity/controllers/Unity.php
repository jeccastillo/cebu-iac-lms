...
class Unity extends CI_Controller {
    ...
    public function registration_viewer_data($id,$sem =  0){
        if($this->is_super_admin() || $this->is_accounting() || $this->is_registrar())
        {
            ...
            $ret['active_sem']['installment1_formatted'] = date("F j, Y", strtotime($ret['active_sem']['installment1']));
            $ret['active_sem']['installment2_formatted'] = date("F j, Y", strtotime($ret['active_sem']['installment2']));
            $ret['active_sem']['installment3_formatted'] = date("F j, Y", strtotime($ret['active_sem']['installment3']));
            $ret['active_sem']['installment4_formatted'] = date("F j, Y", strtotime($ret['active_sem']['installment4']));
            $ret['active_sem']['installment5_formatted'] = date("F j, Y", strtotime($ret['active_sem']['installment5']));
            ...
            if(count($recs) == 0){ //EQUIVALENT
                foreach($equivalent_subjects as $es){
                    ...
                    $this->db->select('floatFinalGrade,strRemarks,tb_mas_subjects.strUnits,tb_mas_subjects.include_gwa,tb_mas_subjects.strCode,intFinalized')
                            ->join('tb_mas_classlist','tb_mas_classlist_student.intClassListID = tb_mas_classlist.intID')  
                            ->join('tb_mas_subjects','tb_mas_classlist.intSubjectID = tb_mas_subjects.intID')                                              
                            ->where(array('tb_mas_classlist.intSubjectID'=>$es['intEquivalentID'],'tb_mas_classlist_student.intStudentID'=>$data['student']['intID'],'tb_mas_classlist_student.strRemarks !='=>'Officially Withdrawn'))                     
                            ->get('tb_mas_classlist_student')
                            ->result_array();        
                    if(count($erecs) > 0){
                        $recs = $erecs;                                                
        ...
        {            
            ...
            if($ret['enlistment']){
                ...
                foreach($enlisted_subjects as $enlisted)
                    $ret['enlisted_subjects'][] = $this->data_fetcher->getClasslistById($enlisted['classlist_id']);
            }
   ...
   public function assign_elective(){
    ...
    public function view_classlist_archive_admin($sem = null, $program = 0, $dissolved = 0, $has_faculty = 0, $status = 0, $modular = 0)
    {
        if($this->is_admin() || $this->is_registrar())
        {
            ...
            $this->data['classlists'] = $this->data_fetcher->fetch_classlists_all(null,$this->data['selected_ay'],'intSubjectID');
    ...
    public function classlist_viewer_data($id,$showAll = 0, $sid){
        ...
        if($this->is_super_admin() || ($this->session->userdata('intID') == $clist['intFacultyID']) || $this->is_registrar())
        {            
    ...
    public function delete_subject_curriculum($type = 0)
    {
        ...
        if($this->is_registrar() || $this->is_super_admin()){
            $post = $this->input->post();            
            $this->data_poster->deleteItem($table,$post['id'],'intID');            
            $data['message'] = "success";
