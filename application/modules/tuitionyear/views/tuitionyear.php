        ...
        <div action="">
            <div class="box ">
                ...
                <div class="box-body" style="padding:2rem">
                    <form @submit.prevent="updateData">
                        <div class="row">
                            <div class="form-group col-xs-6">
                                ...
                                <input v-if="request.final == 0" type="text" name="year" required class="form-control"
                                    id="year" placeholder="Enter Year" v-model='request.year'>
                                <div v-else>{{ request.year }}</div>
                            </div>
                            <div class="form-group col-xs-6">
                                <label for="year">Price Per Unit Regular</label>
                                <input v-if="request.final == 0" type="number" step=".01" name="pricePerUnit" required
                                    class="form-control" id="pricePerUnit" placeholder="Enter Price per unit"
                                    v-model='request.pricePerUnit'>
                                <div v-else>{{ request.pricePerUnit }}</div>
                            </div>
                            <div class="form-group col-xs-6">
                                <label for="year">Price Per Unit Online</label>
                                <input v-if="request.final == 0" type="number" step=".01" name="pricePerUnitOnline"
                                    required class="form-control" id="pricePerUnitOnline"
                                    placeholder="Enter Price per unit" v-model='request.pricePerUnitOnline'>
                                <div v-else>{{ request.pricePerUnitOnline }}</div>
                            </div>
                            <div class="form-group col-xs-6">
                                <label for="year">Price Per Unit Hyflex</label>
                                <input v-if="request.final == 0" type="number" step=".01" name="pricePerUnitHyflex"
                                    required class="form-control" id="pricePerUnitHyflex"
                                    placeholder="Enter Price per unit" v-model='request.pricePerUnitHyflex'>
                                <div v-else>{{ request.pricePerUnitHyflex }}</div>
                            </div>
                            <div class="form-group col-xs-6">
                                <label for="year">Price Per Unit Hybrid</label>
                                <input v-if="request.final == 0" type="number" step=".01" name="pricePerUnitHybrid"
                                    required class="form-control" id="pricePerUnitHybrid"
                                    placeholder="Enter Price per unit" v-model='request.pricePerUnitHybrid'>
                                <div v-else>{{ request.pricePerUnitHybrid }}</div>
                            </div>
                            <div class="form-group col-sm-6">
                                <label for="year">Percent Increase for installment</label>
                                <input v-if="request.final == 0" step="any" type="number" required class="form-control"
                                    placeholder="Enter Percentage" v-model='request.installmentIncrease'>
                                <div v-else>{{ request.installmentIncrease }}</div>
                            </div>
                            <div class="form-group col-sm-6">
                                <label for="year">Percent Down Payment for Installment</label>
                                <input v-if="request.final == 0" step="any" type="number" required class="form-control"
                                    placeholder="Enter Percentage" v-model='request.installmentDP'>
                                <div v-else>{{ request.installmentDP }}</div>
                            </div>
                            <div class="form-group col-sm-6">
                                <label for="year">Fixed Down Payment for Installment (if blank or set to 0 to choose
                                    percent down payment)</label>
                                <input v-if="request.final == 0" step="any" type="number" class="form-control"
                                    placeholder="Enter Fixed Value" v-model='request.installmentFixed'>
                                <div v-else>{{ request.installmentFixed }}</div>
                            </div>
                            <div class="form-group col-sm-6">
                                <label for="year">Number of Free Elective (FOR SHS ONLY)</label>
                                <input v-if="request.final == 0" step="any" type="number" class="form-control"
                                    placeholder="Enter Fixed Value" v-model='request.freeElectiveCount'>
                                <div v-else>{{ request.freeElectiveCount }}</div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-6">
                    ...
                    <hr>
                    <h3>Tuition per Elective (FOR SHS ONLY)</h3>
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            <tr v-for="item in request.elective ">
                                <td>{{ item.strCode + ' ' + item.strDescription }}</td>
                    ...
                    <div v-if="request.final == 0">
                        <p>Add new Tuition for Elective</p>
                        <form @submit.prevent="addExtra('elective','Elective',elective)">
                            <div class="row">
                                <div class="form-group col-sm-3">
                                    <label for="year">Select Elective</label>
                                    <select required class="form-control" @change="selectType($event)"
                                        placeholder="Enter Fee Amount" v-model='elective.subject_id'>
                                        <option v-for="item in selectable_elective_subjects" :value="item.intProgramID">
                                            {{ item.strCode + " " + item.strDescription }}</option>
                                    </select>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">G11 1st Sem</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='elective.tuition_amount'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">G12 1st Sem</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='elective.tuition_amount_online'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">G11 2nd Sem</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='elective.tuition_amount_hyflex'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">G12 2nd Sem</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='elective.tuition_amount_hybrid'>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-sm-6">
                        ...
                        <form @submit.prevent="addExtra('track','Track',track)">
                            <div class="row">
                                <div class="form-group col-sm-3">
                                    ...
                                    <select required class="form-control" @change="selectType($event)"
                                        placeholder="Enter Fee Amount" v-model='track.track_id'>
                                        <option v-for="item in shs_programs" :value="item.intProgramID">
                                            {{ item.strProgramCode + " " + item.strProgramDescription }}</option>
                                    </select>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">G11 1st Sem</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='track.tuition_amount'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">G12 1st Sem</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='track.tuition_amount_online'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">G11 2nd Sem</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='track.tuition_amount_hyflex'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">G12 2nd Sem</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='track.tuition_amount_hybrid'>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-sm-6">
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            <tr v-for="item in request.program">
                                ...
                                <td>{{ item.tuition_amount_hybrid }}</td>
                                <td>{{ item.type }}</td>
                                <td><a v-if="request.final == 0" href="#"
                                        @click.prevent.stop="deleteItem('program',item.id)">Delete</a></td>
                            </tr>
                        </tbody>
                    </table>
                    <hr />
                        ...
                        <form @submit.prevent="addExtra('program','Program',program)">
                            <div class="row">
                                <div class="form-group col-sm-3">
                                    ...
                                    <select required class="form-control" @change="selectType($event)"
                                        placeholder="Enter Fee Amount" v-model='program.track_id'>
                                        <option v-for="item in college_programs" :value="item.intProgramID">
                                            {{ item.strProgramCode + " " + item.strProgramDescription }}</option>
                                    </select>
                                </div>
                                <div class="form-group col-sm-3">
                                    ...
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='program.tuition_amount'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">Online Fee</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='program.tuition_amount_online'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">Hyflex Fee</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='program.tuition_amount_hyflex'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">Hybrid Fee</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='program.tuition_amount_hybrid'>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-sm-6">
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            <tr v-for="item in request.misc">
                                ...
                                <td>{{ item.type }}</td>
                                <td><a v-if="request.final == 0" href="#"
                                        @click.prevent.stop="deleteItem('misc',item.intID)">Delete</a></td>
                            </tr>
                        </tbody>
                    </table>
                    <hr />
                        ...
                        <form @submit.prevent="addExtra('misc','Miscellaneous',misc)">
                            ...
                            <div class="row">
                                <div class="form-group col-sm-3">
                                    <label for="year">Regular Fee</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='misc.miscRegular'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">Online Fee</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='misc.miscOnline'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">Hyflex Fee</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='misc.miscHyflex'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">Hybrid Fee</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='misc.miscHybrid'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">Type</label>
                                    <select required class="form-control" @change="selectType($event)"
                                        placeholder="Enter Fee Amount" v-model='misc.type'>
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            <tr v-for="item in request.lab_fees">
                                ...
                                <td><a v-if="request.final == 0" href="#"
                                        @click.prevent.stop="deleteItem('lab_fee',item.intID)">Delete</a></td>
                            </tr>
                        </tbody>
                    </table>
                    <hr />
                        ...
                        <form @submit.prevent="addExtra('lab_fee','Laboratory',lab)">
                            ...
                            <div class="row">
                                <div class="form-group col-sm-3">
                                    <label for="year">Regular Fee</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='lab.labRegular'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">Online Fee</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='lab.labOnline'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">Hyflex Fee</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='lab.labHyflex'>
                                </div>
                                <div class="form-group col-sm-3">
                                    <label for="year">Hybrid Fee</label>
                                    <input step="any" type="number" required class="form-control"
                                        placeholder="Enter Fee Amount" v-model='lab.labHybrid'>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-sm-6">
                    ...
                    <div v-if="request.intID" class="text-right mt-5">
                        <button @click="finalizeTuition(1)" v-if="request.final == 0"
                            class="btn btn-success">Finalize</button>
                        <button @click="finalizeTuition(0)" v-else-if="special_role == 2"
                            class="btn btn-danger">Un-Finalize</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    mounted() {
        ...
        axios.get('<?php echo base_url(); ?>tuitionyear/tuition_info/' + this.id)
            .then((data) => {
                ...
                this.selectable_elective_subjects = data.data.selectable_elective_subjects;
        ...
        addExtra: function(type, name, data) {
            Swal.fire({
                ...
                preConfirm: (login) => {
                    ...
                    if (type != "track" && type != "program" && type != "elective")
                    ...
                    return axios
                        .post('<?php echo base_url(); ?>tuitionyear/submit_extra/' + type,
                            formdata, {
                                headers: {
        ...
        finalizeTuition: function(type) {
            Swal.fire({
                ...
                preConfirm: (login) => {
                    ...
                    return axios
                        .post('<?php echo base_url(); ?>tuitionyear/finalize_tuition/',
                            formdata, {
                                headers: {
                                    Authorization: `Bearer ${window.token}`
                                }
