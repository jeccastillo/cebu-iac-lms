...
//            $objPHPExcel->getActiveSheet()->setTitle(applicant['strProgramCode'], "-", $student['intStudentYear']);
        ...
        foreach($users as $index => $user)
        {
            ...
            if($reg && substr($user['strStudentNumber'], 0, 1) != 'T'){
                if($reg_status == 'Enrolled'){
                    ...
                    //     print_r($user);
                    //     print_r($tuition);
                    //     print(' assessment_discount_rate_scholar : ' . $assessment_discount_rate_scholar . ' = ' . $tuition['scholarship_total_assessment_rate_scholar']);
                    //     print(' assessment_discount_rate_referrer : ' . $assessment_discount_rate_referrer . ' = ' . $tuition['scholarship_total_assessment_rate_discount']);
                    //     die();
                    // }
                    ...
                    $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('AK'.$i, ($date_enrolled <= $sy->ar_report_date_generation || $tuition['scholarship_total_assessment_rate_scholar'] > 0) && $tuition['scholarship_total_assessment_rate_discount'] > 0 ? $tuition['scholarship_total_assessment_rate_discount'] : '')
    ...
    {
        $asOfDate = $report_date_start == $report_date_end ? date("M d, Y", strtotime($report_date_start)) :  date("M d, Y", strtotime($report_date_start)) . '-' . date("M d, Y", strtotime($report_date_end));
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('A4', 'As of ' . $asOfDate)
                    ->setCellValue('A7', 'No.')
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A2:S7')->applyFromArray(
            array(
                'font'  => array(
                    'bold'  => true,
                    'color' => array('rgb' => '000000'),
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A7:S' . ($i-1))->applyFromArray(
            array(
                'borders' => array(
                    'allborders' => array(
                        'style' => PHPExcel_Style_Border::BORDER_THIN,
        ...
        // Redirect output to a client’s web browser (Excel2007)
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');      
        header('Content-Disposition: attachment;filename="Invoice Report (' . $asOfDate . ').xls"');
        header('Cache-Control: max-age=0');
        // If you're serving to IE 9, then the following may be needed
        header('Cache-Control: max-age=1');
