...
class Clinic extends CI_Controller {
	...
	public function __construct()
	{
        ...
        if($userlevel != 2 && $userlevel != 10 &&  $ip != "172.16.80.22")
    ...
    public function health_records($id = 0)
    {        
        ...
        $this->load->view("health_records",$this->data);
        $this->load->view("common/footer",$this->data);             
        //print_r($this->data['classlist']);
        
    }

    public function health_records_data($id){
    ...
    public function delete_health_record(){
        ...
        if($this->db->where(array('id'=>$post['id']))->delete('tb_mas_health_records')){
            $this->data_poster->log_action('Clinic','Deleted Record: '.$record['last_name'].", ".$record['first_name']." ".$record['chief_complaint'],'red');
            $data['success'] = true;
