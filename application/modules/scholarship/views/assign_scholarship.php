<aside class="right-side" id="registration-container">
    <section class="content-header">
        <h1> Assign Scholarship/Discount </h1>
        <ol class="breadcrumb">
            <li><a :href="base_url + 'scholarship/select_student'"><i
                        class="fa fa-dashboard"></i>Select Student</a></li>
    ...
    <div class="content">
        ...
        <div class="box box-default">
            ...
            <div v-if="registration" class="box-body">
                <div class="row">
                    <div v-if="student" class="col-md-6"> Name: {{ student.strLastname }},
                        {{ student.strFirstname }} {{ student.strMiddlename }}
                    </div>
                    <div class="col-md-6 text-right">
                        <a target="_blank" class="btn btn-primary"
                            :href="base_url + 'scholarship/student_ledger/'+ student.intID">View
                            Ledger</a>
                    </div>
                </div>
                <hr />
                <div class="nav-tabs-custom">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="tab_1">
                            <hr />
                            <div class="row">
                                <div v-if="scholarships && scholarships.length > 0"
                                    class="col-md-6">
                                    <h4>Assign Scholarship</h4>
                                    <form method="post" action="#"
                                        @submit.prevent.stop="submitDeduction('scholarship')">
                                        <label>Select Scholarship</label>
                                        <select required class="form-control"
                                            v-model="request_scholarship.discount_id">
                                            <option v-for="scholarship in scholarships"
                                                :value="scholarship.intID">{{ scholarship.name }}
                                            </option>
                                        </select>
                                        <hr />
                                ...
                                <div v-if="discounts && discounts.length > 0" class="col-md-6">
                                    <h4>Assign Discount</h4>
                                    <form method="post" action="#"
                                        @submit.prevent.stop="submitDeduction('discount')">
                                        <label>Select Discount</label>
                                        <select required class="form-control"
                                            v-model="request_discount.discount_id">
                                            <option v-for="discount in discounts"
                                                :value="discount.intID">{{ discount.name }}</option>
                                        </select>
                                        <hr />
                                        <label>Referree Name</label>
                                        <select class="form-control"
                                            v-model="request_discount.referrer">
                                            <option v-for="st in students"
                                                :value="st.strLastname+', '+st.strFirstname+' '+st.strMiddlename">
                                                {{ st.strLastname+', '+st.strFirstname+' '+st.strMiddlename }}
                                            </option>
                                        </select>
                                        <hr />
                                        <input class="btn btn-primary" type="submit" value="Add">
                                ...
                                <table class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        <tr v-for="item in student_scholarships">
                                            ...
                                            <td>
                                                <select class="form-control"
                                                    @change="updateScStatus($event,item.id)">
                                                    <option :selected="item.status == 'applied'"
                                                        value="applied">applied</option>
                                                    <option :selected="item.status == 'pending'"
                                                        value="pending">pending</option>
                                                    <option :selected="item.status == 'withdraw'"
                                ...
                                <table class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        <tr v-for="item in student_discounts">
                                            <td>{{ item.name }}</td>
                                            <td>{{ item.referrer=='undefined'?'not applicable':item.referrer }}
                                            </td>
                                            <td>
                                                <select class="form-control"
                                                    @change="updateScStatus($event,item.id)">
                        ...
                        <div class="tab-pane" id="tab_2">
                            ...
                            <table class="table table-bordered table-striped">
                                ...
                                <tr>
                                    <th colspan="8"> Other Payments: </th>
                                </tr>
                                ...
                                <tr>
                                    <th colspan="8"> Tuition Payments: </th>
                                </tr>
                                ...
                                <tr>
                                    <td class="text-green" colspan="8"> amount paid:
                                        P{{ amount_paid_formatted }}
                                    </td>
                                </tr>
                                <tr>
                                    <td class="text-green" colspan="8"> remaining balance:
...
new Vue({
    ...
    mounted() {
        ...
        axios.get(this.base_url + 'scholarship/assign_scholarship_data/' + this.student_id +
            '/' + this.current_sem).then((data) => {
            ...
            axios.get(api_url + 'finance/transactions/' + this.student.slug + '/' +
                this.current_sem).then((data) => {
                this.payments = data.data.data;
                this.other_payments = data.data.other;
                if (this.registration && this.registration.paymentType ==
                    'partial') this.has_partial = true;
                if (this.has_partial) this.remaining_amount = this
                ...
                for (i in this.payments) {
                    if (this.payments[i].status == "Paid") {
                        ...
                        this.amount_paid = this.amount_paid + this.payments[
                            i].subtotal_order;
                    }
                }
                if (this.registration.enumStudentType == "new") {
                    axios.get(api_url + 'finance/reservation/' + this
                        .student.slug + '/' + this.current_sem).then((
                        data) => {
                        ...
                        for (i in this.reservation_payments) {
                            if (this.reservation_payments[i]
                                .status == "Paid" && data.data
                                .student_sy == this.current_sem) {
                                ...
                                this.amount_paid = this
                                    ...
                                    .reservation_payments[i]
                                    .subtotal_order;
                                this.tuition_data.down_payment =
                                    this.tuition_data.down_payment -
                    ...
                    this.remaining_amount = (this.remaining_amount < 0.02) ?
                    ...
                    if (this.registration.downpayment == 1) {
                        ...
                        for (i = 0; i < 5; i++) {
                            if (this.tuition_data.installment_fee > temp) {
                                val = this.tuition_data.installment_fee -
                                    temp;
                                val = val.toFixed(2);
                                this.installments.push(val);
                                temp = 0;
                        ...
                        for (i = 0; i < 5; i++) this.installments.push(this
                            .tuition_data.installment_fee);
                    var val = 0;
                    this.amount_paid_formatted = this.amount_paid.toFixed(2)
                        .replace(/\d(?=(\d{3})+\.)/g, '$&,');
                    this.loader_spinner = false;
                    if (this.remaining_amount <= 0) this.description =
                        "Other";
                }
            }).catch((error) => {
                console.log(error);
    ...
    methods: {
        changeTerm: function(event) {
            document.location = base_url + 'scholarship/assign_scholarship/' + event
                .target.value + '/' + this.student_id;
        },
        enrolledStudents: function() {
            this.referee_names = [];
            axios.get(this.base_url + 'scholarship/enrolled_student_data?search=' + this
                .refereee_search_name).then((data) => {
        ...
        onChangeRefereee(data) {
            ...
            this.request_discount.referrer = this.referee_data.strLastname + ', ' + this
                .referee_data.strFirstname + ' ' + this.referee_data.strMiddlename;
        },
        ...
        submitDeduction: function(type) {
            var req = {};
            var formdata = new FormData();
            if (type == "scholarship") req = this.request_scholarship;
            else req = this.request_discount;
