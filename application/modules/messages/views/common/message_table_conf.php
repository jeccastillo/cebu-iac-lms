...
<script type="text/javascript">
    ...
    $('.mark-as-read').click(function(e){
        ...
        if(objects.length > 0 ){
            ...
            $.ajax({
                'url':'<?php echo base_url(); ?>messages/markMessages/1',
                'method':'post',
                'data':data,
                'dataType':'json',
    ...
    $('.mark-as-unread').click(function(e){
        ...
        if(objects.length > 0 ){
            var data = {'ids':objects};
            $.ajax({
                'url':'<?php echo base_url(); ?>messages/markMessages/0',
                'method':'post',
                'data':data,
                'dataType':'json',
                'success':function(ret){
    ...
    $('#messages_table').dataTable( {
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo base_url(); ?>datatables/data_tables_ajax/tb_mas_message_user/<?php echo $user['intID']; ?>/0",
            "aoColumnDefs":[
                {
                    "aTargets":[5],
                    "mData": null,
                    "bSortable":false,
                    "mRender": function (data,type,row,meta) { return '<?php echo dropdown_menu_open(); ?><li><a href="<?php echo base_url(); ?>messages/view_message/'+row[0]+'">Read</a><li><li><a href="#" rel="'+row[0]+'" class="trash-message">Send to Trash</a><li><li><a href="<?php echo base_url(); ?>messages/mark_unread/'+row[0]+'">Mark as Unread</a><li><?php echo dropdown_menu_close(); ?>'; }
                },
                {
                    "aTargets":[0],
                    "bVisible": false 
            ...
            "oLanguage": {"sZeroRecords": "No Messages in Inbox", "sEmptyTable": "No Messages in Inbox"},
            "aaSorting": [[4,'desc']],
