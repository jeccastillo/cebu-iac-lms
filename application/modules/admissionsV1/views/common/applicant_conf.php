...
$(document).ready(function() {
  ...
  var dtable = $('#applicant-table').dataTable({
    ...
    ajax: function(data, callback, settings) {
      ...
      $.get(
        api_url + "admissions/applications/paid-application-fee" + daterange, {
          limit: data.length,
    ...
    "aoColumnDefs": [{
        ...
        "mRender": function(data, type, row, meta) {
          return '<?php echo $d_open; ?><li><a href="<?php echo base_url(); ?>admissionsV1/view_lead/' +
            row.slug + '">View Details</a></li></ul></div>';
        }
      },
      {
        "aTargets": [0],
  ...
  $("#select-term-leads").on('change', function(e) {
    const term = $(this).val();
    document.location = "<?php echo base_url()."admissionsV1/paid_applicants/"; ?>" + term;
  });
