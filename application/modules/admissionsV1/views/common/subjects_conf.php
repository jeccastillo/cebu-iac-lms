...
$(document).ready(function() {
        ...
        ajax: function(data, callback, settings) {
            ...
            $.get(api_url + "admissions/applications" + daterange, {
                ...
                $("#print_form").click(function(e) {
                    e.preventDefault();
                    axios.get(
                            'https://smsapi.iacademy.edu.ph/api/v1/sms/admissions/student-info/view-applicants/' +
        ...
        "aoColumnDefs": [{
            ...
            "mRender": function(data, type, row, meta) {
                return '<?php echo $d_open; ?><li><a target="_blank" href="<?php echo base_url(); ?>admissionsV1/view_lead_new/' +
                    row.slug + '">View Details</a></li>' +
                    '<li><a target="_blank" href="<?php echo base_url(); ?>finance/manualPay/' +
                    row.slug + '">Finance Viewer</a></li>' +
                    '<li><a target="_blank" href="<?php echo base_url(); ?>admissionsV1/app_form/' +
                    row.slug + '">Application Form</a></li></ul></div>';
            }
        }, {
            "aTargets": [0],
            "bVisible": false
    ...
    $('#daterange-btn-users').daterangepicker({
        ...
        daterange = "?start=" + start.format('YYYY-MM-D') + '&end=' + end.format(
            'YYYY-MM-D') + '&range_field=' + $("#range-to-select").val();
        dtable.fnDraw(false);
    });
    ...
    $("#select-term-leads").on('change', function(e) {
        const term = $(this).val();
        document.location =
            "<?php echo base_url()."admissionsV1/view_all_leads/"; ?>" + term;
    });
});
</script>