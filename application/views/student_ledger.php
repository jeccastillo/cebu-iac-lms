                    ...
                    <table v-for="(term,i) in ledger" v-if="term.ledger_items.length > 0" class="table table-bordered table-striped">
                        ...
                        <tbody>                                                         
                            <tr v-for="(item,j) in term.ledger_items">                                
                                ...
                                <td :class="item.muted">{{ (item.type == 'payment' || (item.type == 'balance' && item.amount < 0))? (item.amount < 0 ? numberWithCommas(item.amount * -1) : numberWithCommas(item.amount)) :'-' }}</td>
    ...
    methods: { 
        ...
        getPayments: function(tuition){
            ...
            if (this.ledger_term && this.ledger_term.length > 1) {
                ...
                const rest = this.ledger_term.slice(1).sort((a, b) => {
                    const aTime = a && a.date ? new Date(a.date).getTime() : Number.POSITIVE_INFINITY;
                    const bTime = b && b.date ? new Date(b.date).getTime() : Number.POSITIVE_INFINITY;
                    return aTime - bTime;
                });
