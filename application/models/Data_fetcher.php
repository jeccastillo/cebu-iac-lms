...
//        
    ...
    {
        $subjects = $this->db
                         ->select('tb_mas_curriculum_second.intID,subj1.strCode,subj1.strUnits,subj1.intID as intSubjectID,subj1.strDescription,subj1.intLab, subj1.intLectHours,subj1.strUnits,subj1.include_gwa,tb_mas_curriculum_second.type, subj2.strCode as strCodeEquivalent, tb_mas_curriculum_second.combineCode, tb_mas_curriculum_second.combineDesc')
                         ->from('tb_mas_curriculum_second')
                         ->join('tb_mas_subjects subj1','subj1.intID = tb_mas_curriculum_second.intSubjectID')
                         ->join('tb_mas_subjects subj2','subj2.intID = tb_mas_curriculum_second.equivalentSubjectID', 'left')
                         ->where('tb_mas_curriculum_second.intCurriculumID',$id)
                         ->order_by('subj1.strCode asc')
                         ->get()
                         ->result_array();
...
//                     ->join('tb_mas_registration','tb_mas_registration.intAYID = tb_mas_sy.intID and tb_mas_registration.intStudentID = tb_mas_classlist_student.intStudentID')
    ...
    function getTuition($id,$sem,$sch = 0,$discount = 0)
    {
        ...
        $subjects =  $this->db
                            ->select("tb_mas_subjects.intID as subjectID,tb_mas_classlist.is_modular,tb_mas_classlist.is_special_class,tb_mas_classlist.payment_amount,tb_mas_subjects.intMajor,tb_mas_subjects.isElective, tb_mas_classlist_student.additional_elective")
                            ->from("tb_mas_classlist_student")
            ...
            foreach($discounts as $scholar){
                ...
                if($scholar->total_assessment_rate > 0 || $scholar->total_assessment_fixed > 0){                                    
                    ...
                    if($scholar->tuition_fee_rate > 0){                        
                        ...
                        $tuition_scholarship_installment_current = ($tuition + ($tuition * ($tuition_year['installmentIncrease']/100)) - $scholarship_installment_grand_total - $tuition_discount_installment) * ($scholar->tuition_fee_rate/100);
                        $tuition_scholarship_installment_current30 = ($tuition + ($tuition * 0.15) - $scholarship_installment_grand_total30 - $tuition_discount_installment30) * ($scholar->tuition_fee_rate/100);
                        $tuition_scholarship_installment_current50 = ($tuition + ($tuition * 0.09) - $scholarship_installment_grand_total50 - $tuition_discount_installment50) * ($scholar->tuition_fee_rate/100);
                        ...
                        // print($total_assessment_rate_discount_installment50 . ' @ ');
