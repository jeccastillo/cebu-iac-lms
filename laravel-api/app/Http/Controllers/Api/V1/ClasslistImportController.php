...
class ClasslistImportController extends Controller
{
    ...
    /**
     * GET /api/v1/classlists/import/template
     * Role: registrar,admin
     * Returns an .xlsx template with headers for classlists import.
     */
    public function template(Request $request)
    {
        // Wrap service builder via export class for parity with other templates
        $export = new ClasslistTemplateExport($this->service);
        $spreadsheet = $export->build();
        $writer = new Xlsx($spreadsheet);

        $filename = 'classlists-import-template.xlsx';
    ...
    public function import(Request $request): JsonResponse
    {
        try {
            ...
            if ($tmpPath === false || $tmpPath === null) {
                $tmpPath = sys_get_temp_dir() . DIRECTORY_SEPARATOR . uniqid('classlists_import_', true) . '.' . $ext;
