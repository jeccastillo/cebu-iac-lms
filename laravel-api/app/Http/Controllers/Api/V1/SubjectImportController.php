...
use App\Http\Requests\Api\V1\SubjectImportRequest;
...
class SubjectImportController extends Controller
{
     ...
     * Returns an .xlsx template with headers for subjects import.
     */
    public function template(Request $request)
    {
        $export = new SubjectTemplateExport($this->service);
        $spreadsheet = $export->build();
        $writer = new Xlsx($spreadsheet);

        $filename = 'subjects-import-template.xlsx';
    ...
    public function import(SubjectImportRequest $request): JsonResponse
    {
        try {
            ...
            if ($tmpPath === false || $tmpPath === null) {
                $tmpPath = sys_get_temp_dir() . DIRECTORY_SEPARATOR . uniqid('subjects_import_', true) . '.' . $ext;
