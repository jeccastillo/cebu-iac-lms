  ...
  function ClassroomsService($http, APP_CONFIG, StorageService) {
    ...
    return {
      list: function (q) {
        ...
        return $http.get(BASE + '/classroom', { params: params })
          .then(_unwrap);
      },
      get: function (id) {
        return $http.get(BASE + '/classroom/' + encodeURIComponent(id))
          .then(_unwrap);
      },
      create: function (payload) {
        return $http.post(BASE + '/classroom', payload, _adminHeaders())
          .then(_unwrap);
      },
      update: function (id, payload) {
        return $http.put(BASE + '/classroom/' + encodeURIComponent(id), payload, _adminHeaders())
          .then(_unwrap);
      },
      delete: function (id) {
        return $http.delete(BASE + '/classroom/' + encodeURIComponent(id), _adminHeaders())
          .then(_unwrap);
      }
    };
  }
