(function () {
  ...
  function SchoolYearsListController($location, $window, StorageService, SchoolYearsService, CampusService) {
      ...
      try {
        SchoolYearsService.downloadImportTemplate()
          .then(function (res) {
            ...
            var filename = (res && res.filename) ? res.filename : 'school-years-import-template.xlsx';
            if (!data) {
    ...
    vm.openImportDialog = function () {
      vm.importError = null;
      vm.importSummary = null;
      var el = document.getElementById('schoolYearsImportFile');
      if (el) {
        try {
    ...
    vm.runImport = function () {
      ...
      SchoolYearsService.importFile(vm._selectedFile, { dry_run: false })
        .then(function (res) {
