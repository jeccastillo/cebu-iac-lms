(function () {
  ...
  angular
    .module('unityApp')
    .factory('InvoiceReportsService', InvoiceReportsService);

  InvoiceReportsService.$inject = ['$http', 'APP_CONFIG', 'StorageService', 'CampusService'];
  function InvoiceReportsService($http, APP_CONFIG, StorageService, CampusService) {
    ...
    /**
     * Fetch invoice reports for a date range (filters use payment_details.or_date).
     * @param {{date_from: string|Date, date_to: string|Date, type?: string, status?: string}} filters
    ...
    function list(filters) {
      ...
      return $http.get(BASE + '/reports/invoices', cfg).then(function (resp) {
