(function () {
  ...
  angular
    .module('unityApp')
    .controller('ApplicantTypesController', ApplicantTypesController)
    .controller('ApplicantTypeEditController', ApplicantTypeEditController);

  ApplicantTypesController.$inject = ['$location', 'ApplicantTypesService'];
  function ApplicantTypesController($location, ApplicantTypesService) {
    var vm = this;

    vm.title = 'Applicant Types';
    vm.error = null;
    vm.total = 0;
    ...
    function load() {
      vm.loading = true;
      vm.error = null;
      ApplicantTypesService.list(vm.filters)
        .then(function (res) {
        ...
        .catch(function (err) {
          ...
          vm.meta.last_page = 1;
          vm.meta.total = 0;
          vm.error = (err && (err.message || (err.data && err.data.message))) || 'Failed to load applicant types.';
        })
    ...
    function add() {
      $location.path('/admissions/applicant-types/new');
    }

    function edit(row) {
      if (!row || (row.id === undefined || row.id === null)) return;
      $location.path('/admissions/applicant-types/' + row.id + '/edit');
    }
    ...
    function removeItem(row) {
      ...
      var doRemove = function () {
        vm.loading = true;
        ApplicantTypesService.remove(row.id)
          .then(function () {
      ...
      if (window.Swal) {
        ...
        if (confirm('Delete this record?')) doRemove();
      }
    }
  ...
  ApplicantTypeEditController.$inject = ['$routeParams', '$location', 'ApplicantTypesService'];
  function ApplicantTypeEditController($routeParams, $location, ApplicantTypesService) {
    var vm = this;
    vm.loading = false;
    ...
    function load() {
      vm.loading = true;
      ApplicantTypesService.show(vm.id)
        .then(function (res) {
    ...
    function save() {
      ...
      if (!vm.model.type) {
        if (window.Swal) Swal.fire({ icon: 'error', title: 'Missing required field', text: 'Type is required.' });
        return;
      }
      ...
      var payload = {
        name: vm.model.name,
        type: vm.model.type,
        sub_type: vm.model.sub_type
      };

      vm.loading = true;
      var p = vm.isCreate
        ? ApplicantTypesService.create(payload)
        : ApplicantTypesService.update(vm.id, payload);

      p.then(function () {
          if (window.Swal) Swal.fire({ icon: 'success', title: 'Saved' });
          $location.path('/admissions/applicant-types');
        })
        .catch(function (err) {
