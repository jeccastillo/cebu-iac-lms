(function () {
  ...
  angular
    .module('unityApp')
    .controller('PreviousSchoolsController', PreviousSchoolsController)
    .controller('PreviousSchoolEditController', PreviousSchoolEditController);

  PreviousSchoolsController.$inject = ['$location', 'PreviousSchoolsService'];
  function PreviousSchoolsController($location, PreviousSchoolsService) {
    var vm = this;

    vm.title = 'Previous Schools';
    vm.error = null;
    vm.total = 0;
    ...
    function load() {
      vm.loading = true;
      vm.error = null;
      PreviousSchoolsService.list(vm.filters)
        .then(function (res) {
        ...
        .catch(function (err) {
          ...
          vm.meta.last_page = 1;
          vm.meta.total = 0;
          vm.error = (err && (err.message || (err.data && err.data.message))) || 'Failed to load previous schools.';
        })
    ...
    function add() {
      $location.path('/admissions/previous-schools/new');
    }

    function edit(row) {
      if (!row || (row.id === undefined || row.id === null)) return;
      $location.path('/admissions/previous-schools/' + row.id + '/edit');
    }
    ...
    function removeItem(row) {
      ...
      var doRemove = function () {
        vm.loading = true;
        PreviousSchoolsService.remove(row.id)
          .then(function () {
  ...
  PreviousSchoolEditController.$inject = ['$routeParams', '$location', 'PreviousSchoolsService'];
  function PreviousSchoolEditController($routeParams, $location, PreviousSchoolsService) {
    var vm = this;
    vm.loading = false;
    ...
    function load() {
      vm.loading = true;
      PreviousSchoolsService.show(vm.id)
        .then(function (res) {
    ...
    function save() {
      ...
      var payload = {
        ...
        province: vm.model.province || null,
        country: vm.model.country || null
      };

      vm.loading = true;
      var p = vm.isCreate
        ? PreviousSchoolsService.create(payload)
        : PreviousSchoolsService.update(vm.id, payload);

      p.then(function () {
          if (window.Swal) Swal.fire({ icon: 'success', title: 'Saved' });
          $location.path('/admissions/previous-schools');
        })
        .catch(function (err) {
